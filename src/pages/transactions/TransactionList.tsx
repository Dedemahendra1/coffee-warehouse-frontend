...
const TransactionList = () => {
  const { user } = useAuth();
  const hasOutlet = !!user?.outlet?.id;
  const { data: outlet } = useMyOutletProfile();

  const {
    data: transactions = [], 
  } = useFetchOutletTransactions({
    enabled: hasOutlet,
  });
            ...
            >
              ...
              >
                ...
                {transactions.length > 0 ? (
                  transactions.map((tx) => (
                    <div className="card-outlet flex flex-col rounded-2xl border border-monday-border">
