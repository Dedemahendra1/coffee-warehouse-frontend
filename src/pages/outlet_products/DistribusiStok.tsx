...
import {
  DistribusiStokFormData,
  distribusiStokSchema,
} from "../../schemas/distribusiStokSchema";
import { AxiosError } from "axios";
import { useDistribusiStok } from "../../hooks/useOutletProducts";
import { ApiErrorResponse, DistribusiStokPayload } from "../../types/types";
import UserProfileCard from "../../components/UserProfileCard";
import { useAuth } from "../../hooks/useAuth";

const DistribusiStok = () => {
  const { id } = useParams<{ id: string }>();
  const { user } = useAuth();
  const isKeeper = user?.roles?.includes("keeper");

  const { data: managerOutlet, isPending: loadingManager } = useFetchOutlet(
    Number(id), { enabled: !isKeeper }
  );
  const { data: keeperOutlet, isPending: loadingKeeper } = useMyOutletProfile(
    { enabled: !!isKeeper }
  );
  ...
  const { mutate: assignProduct, isPending } = useDistribusiStok();
  ...
  const {
    ...
    setError,
    formState: { errors },
  } = useForm<DistribusiStokFormData>({
    resolver: zodResolver(distribusiStokSchema),
  });

  const onSubmit = (data: DistribusiStokFormData) => {
    setError("root", { type: "server", message: "" });

    const payload: DistribusiStokPayload = {
      outlet_id: Number(outlet?.id ?? id),
        ...
        if (error.response) {
          ...
          if (errors) {
            Object.entries(errors).forEach(([key, messages]) => {
              setError(key as keyof DistribusiStokFormData, {
                type: "server",
                message: messages[0],
              });
            });
  ...
  if (loadingOutlet || loadingProducts || loadingWarehouses)
    return <p>Loading...</p>;
          ...
          <div className="flex gap-6">
            <div className="flex flex-col gap-6 w-full">
              <div className="flex flex-col rounded-3xl p-[18px] gap-5 bg-white">
                <p className="font-semibold text-xl">Outlet Details</p>
              ...
              >
                ...
                <Link to={`/outlet-products/${outlet.id}`}
              ...
              <ul className="flex flex-col gap-4">
                <li className="flex gap-[6px]">
                  ...
                  <p className="font-medium leading-[140%]">
                    Verify and confirm outlet Lorem information to ensure
                    accuracy
                  </p>
                </li>
                <li className="flex gap-[6px]">
