...
import { DistribusiStokWarehouseFormData, distribusiStokWarehouseSchema } from "../../schemas/distribusiStokWarehouseSchema";
import { useFetchProducts } from "../../hooks/useProducts";
import { useAssignWarehouseProduct } from "../../hooks/useWarehouseProducts";
import { ApiErrorResponse, DistribusiStokWarehousePayload } from "../../types/types";
import UserProfileCard from "../../components/UserProfileCard";

const DistribusiStokWarehouse = () => {
  ...
  const {
    ...
    setError,
    formState: { errors },
  } = useForm<DistribusiStokWarehouseFormData>({
    resolver: zodResolver(distribusiStokWarehouseSchema),
  });

  const onSubmit = (data: DistribusiStokWarehouseFormData) => {

    setError("root", { type: "server", message: "" });

    const payload: DistribusiStokWarehousePayload = {
        ...
        if (error.response) {
          ...
          if (errors) {
            Object.entries(errors).forEach(([key, messages]) => {
              setError(key as keyof DistribusiStokWarehouseFormData, { type: "server", message: messages[0] });
            });
          }
        } 
      },
...
export default DistribusiStokWarehouse;
