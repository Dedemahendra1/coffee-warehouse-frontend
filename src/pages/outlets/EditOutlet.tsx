...
import { useFetchOutlet, useUpdateOutlet } from "../../hooks/useOutlets";
import { useFetchUsers } from "../../hooks/useUsers";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { OutletFormData, outletSchema } from "../../schemas/outletSchema";
...
const EditOutlet = () => {
  ...
  const { data: outlet, isPending: outletPending } = useFetchOutlet(
    Number(id)
  ); // ✅ Use `isPending`

  const { mutate: updateOutlet, isPending } = useUpdateOutlet(); // ✅ Use `isPending`
  ...
  // ✅ Populate form fields when outlet data is available
  useEffect(() => {
    if (outlet) {
      setValue("name", outlet.name);
        setValue("phone", outlet.phone);
        setValue("address", outlet.address);
        setValue("keeper_id", outlet.keeper_id);
        if (outlet.photo) {
          setImagePreview(outlet.photo);  
        } 
    }
  }, [outlet, setValue]); 
        ...
        onError: (error: AxiosError<ApiErrorResponse>) => {
          ...
          if (fieldErrors) {
            Object.entries(fieldErrors).forEach(([key, value]) => {
              setError(key as keyof OutletFormData, {
                type: "server",
                message: value[0],
              });
            });
  ...
  if (outletPending) return <p>Loading outlet details...</p>;
        ...
        >
          <div className="flex items-center gap-6 h-[92px] bg-white w-full rounded-3xl p-[18px]">
            <div className="flex flex-col gap-[6px] w-full">
              <h1 className="font-bold text-2xl">Edit Outlet</h1>
              <Link to={'/outlets'}
        ...
        <main className="flex flex-col gap-6 flex-1">
          <div className="flex gap-6">
            ...
            <div className="flex flex-col w-[392px] shrink-0 h-fit rounded-3xl p-[18px] gap-3 bg-white">
              <p className="font-semibold">Quick Guide to Add New Outlet</p>
              <ul className="flex flex-col gap-4">
                <li className="flex gap-[6px]">
                  ...
                  <p className="font-medium leading-[140%]">
                    Use a clear and recognizable outlet name Lorem Ipsum mix
                    amet
                  </p>
                </li>
                <li className="flex gap-[6px]">
                ...
                <li className="flex gap-[6px]">
                  ...
                  />
                  <p className="font-medium leading-[140%]">
                    Upload a high-quality outlet photo for easy identification
                  </p>
                </li>
              </ul>
            </div>
